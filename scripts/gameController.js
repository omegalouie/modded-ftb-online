...
function drawTank() {
	...
	if (shape === "mothership") {
		ctx.globalAlpha = tankalpha;
		if (editmode === false) {
			drawPoly(tankpointx, tankpointy, tanksize, (angle(tankpointx, tankpointy, mouse.x, mouse.y)), document.getElementById("color").value, 16)
		} else {
			drawPoly(tankpointx, tankpointy, tanksize, 0, document.getElementById("color").value, 16)
		}
}
	if (shape === "spike") {
		ctx.save();
	...
	if (shape === "landmine2") {
		...
		ctx.beginPath();
		if (editmode === false) {
			ctx.rotate((angle(tankpointx, tankpointy, mouse.x, mouse.y) * (Math.PI / 180)) * 1.5);
		}
		ctx.moveTo(0, tanksize + (tanksize / 3));
		ctx.rotate((360 / 6) * (Math.PI / 180));
		ctx.lineTo(0, tanksize + (tanksize / 3));
		...
		ctx.beginPath();
		if (editmode === false) {
			ctx.rotate((angle(tankpointx, tankpointy, mouse.x, mouse.y) * (Math.PI / 180)) * 0.5);
		}
		ctx.moveTo(0, tanksize + (tanksize / 3));
		ctx.rotate((360 / 6) * (Math.PI / 180));
		ctx.lineTo(0, tanksize + (tanksize / 3));
	...
	if (shape === "build") {
		ctx.globalAlpha = tankalpha;
		if (editmode === false) {
			drawConc(tankpointx, tankpointy, tanksize+4, (angle(tankpointx, tankpointy, mouse.x, mouse.y)+45), document.getElementById("color").value, 4, (tanksize+4)/1.6)
			} else {
			drawConc(tankpointx, tankpointy, tanksize+4, 45, document.getElementById("color").value, 4, (tanksize+4)/1.6)
		}
}
	if (shape === "star") {
		ctx.globalAlpha = tankalpha;
		if (editmode === false) {
			drawConc(tankpointx, tankpointy, tanksize+5, (angle(tankpointx, tankpointy, mouse.x, mouse.y)+270), document.getElementById("color").value, 5, (tanksize+5)/2)
			} else {
			drawConc(tankpointx, tankpointy, tanksize+5, 270, document.getElementById("color").value, 5, (tanksize+5)/2)
		}
}
	if (shape === "convex3") {
		ctx.globalAlpha = tankalpha;
		if (editmode === false) {
			drawConc(tankpointx, tankpointy, tanksize+3, (angle(tankpointx, tankpointy, mouse.x, mouse.y)+270), document.getElementById("color").value, 3, (tanksize+3)/1.5)
			} else {
	...
	if (shape === "boomer") {
		ctx.globalAlpha = tankalpha;
		if (editmode === false) {
			drawConc(tankpointx, tankpointy, tanksize+5, (angle(tankpointx, tankpointy, mouse.x, mouse.y)+270), document.getElementById("color").value, 5, (tanksize+5)/1.3)
			} else {
			drawConc(tankpointx, tankpointy, tanksize+5, 270, document.getElementById("color").value, 5, (tanksize+5)/1.3)
		}
}
	if (shape === "rect") {
		ctx.globalAlpha = tankalpha;
	...
	if (shape === "drive") {
		...
		drawBullet(tankpointx, tankpointy, tanksize, tankalpha);
		if (editmode === false) {
			drawPoly(tankpointx, tankpointy, tanksize/1.5, (angle(tankpointx, tankpointy, mouse.x, mouse.y)+45), document.getElementById("bcolo").value, 4)
			} else {
...
function setBColor(bcolo) {
	document.getElementById("bcolo").value = bcolo;
}
